        ...
        NavigationStack {
            ZStack {
                ...
                if store.items.isEmpty {
                    ContentUnavailableView("No Trips Yet", systemImage: "tray", description: Text("Tap + to log your first one."))
                } else {
                    List {
                        ForEach(store.items) { item in
                            HStack {
                                VStack(alignment: .leading, spacing: 4) {
                                    ...
                                    Text("Song Count: " + String(format: "%.1f", item.value2))
            ...
            .navigationTitle("Roadtrip Tunes")
            .toolbar {
...
struct AddItemView: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    TextField("Trip Name", text: $titleText)
                        .focused($focusedField, equals: .title)
                        .accessibilityIdentifier("addTitleField")
                    TextField("Song Count", text: $value2Text)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        let item = Trip(title: titleText.isEmpty ? "Trip" : titleText, value2: Double(value2Text) ?? 0, notes: notesText)
                        if store.add(item) {
                            dismiss()
                        }
                    }
