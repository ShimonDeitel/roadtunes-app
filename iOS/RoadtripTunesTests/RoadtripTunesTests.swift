import XCTest
@testable import RoadtripTunes

@MainActor
final class RoadtripTunesTests: XCTestCase {
    var store: Store!
    ...
    func testAddItemIncreasesCount() {
        ...
        let added = store.add(Trip(title: "Test", value2: 0))
        XCTAssertTrue(added)
    ...
    func testCannotAddBeyondFreeLimitWhenNotPro() {
        ...
        for i in 0..<Store.freeLimit {
            _ = store.add(Trip(title: "Item \(i)", value2: 0))
        }
        let result = store.add(Trip(title: "Overflow", value2: 0))
    ...
    func testDeleteRemovesItem() {
        let item = Trip(title: "ToDelete", value2: 0)
        _ = store.add(item)
    ...
    func testUpdateModifiesItem() {
        var item = Trip(title: "Original", value2: 0)
        _ = store.add(item)
        item.title = "Updated"
        store.update(item)
    ...
    func testDeleteAtOffsets() {
        _ = store.add(Trip(title: "A", value2: 0))
        _ = store.add(Trip(title: "B", value2: 0))
    ...
    func testPersistenceRoundTrip() {
        _ = store.add(Trip(title: "Persisted", value2: 0))
        store.save()
        let reloaded = Store()
